...
function setupPageFunctionality() {
    ...
    switch (currentPage) {
        ...
        case 'dashboard.html':
            ...
            setupDashboardPage(); break;
        case 'ai-tutor.html':
            if (!currentUser) return navigateTo('login.html');
            setupAITutorPage(); break;
        case 'wallet.html':
            if (!currentUser) return navigateTo('login.html');
            setupWalletPage(); break;
        case 'admin.html':
            if (!currentUser || currentUser.role !== 'admin') return navigateTo('dashboard.html');
            setupAdminPage(); break;
...
function setupDashboardPage() { updateDashboard(); }
...
// ---------------- Course Filtering ----------------
function filterCoursesData() { return mockCourses; } // Simple: returns all
function filterCourses() { populateCourses(); }
...
function updateDashboard() {
    const enrolledCourses = mockCourses.filter(c => c.enrolled).length;
    const enrolledCountEl = document.getElementById('enrolled-courses');
    if (enrolledCountEl) enrolledCountEl.textContent = enrolledCourses;
}

// ---------------- Wallet ----------------
function toggleWalletAddress() {
    const walletAddress = document.getElementById('wallet-address');
    const btn = document.getElementById('toggle-address');
    if (!walletAddress || !btn) return;

    if (walletAddress.textContent.includes('•')) {
        walletAddress.textContent = '1FfmbHfnpaZjKFvyi1okTjJJusN455paPH'; // Example address
...
function sendMessage() {
    ...
    setTimeout(() => {
        ...
        aiDiv.textContent = "I'm here to help you with cybersecurity. Can you clarify your question?";
...
function handleLogin(email, password) {
    ...
    currentUser = { name: email.split('@')[0], email, role: email.includes('admin') ? 'admin' : 'user' };
...
function handleRegister(name, email, password, confirmPassword) {
    ...
    // Mock registration: simply login the user
...
function setupAdminPage() {
    const manageUsersBtn = document.querySelector('button:contains("Manage Users")');
    const manageCoursesBtn = document.querySelector('button:contains("Manage Courses")');
    const processRefundsBtn = document.querySelector('button:contains("Process Refunds")');
    const viewAnalyticsBtn = document.querySelector('button:contains("View Analytics")');

    if (manageUsersBtn) manageUsersBtn.addEventListener('click', () => alert('User management is not implemented yet.'));
    if (manageCoursesBtn) manageCoursesBtn.addEventListener('click', () => alert('Course management is not implemented yet.'));
    if (processRefundsBtn) processRefundsBtn.addEventListener('click', () => alert('Refunds are not implemented yet.'));
    if (viewAnalyticsBtn) viewAnalyticsBtn.addEventListener('click', () => alert('Analytics are not implemented yet.'));
}
